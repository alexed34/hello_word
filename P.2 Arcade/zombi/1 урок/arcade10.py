...
class PeaShooter(Plant):  # класс Горохострел
    def __init__(self):
        ...
        #self.pea_spawn_sound = arcade.load_sound("peaspawn.mp3")
    ...
    def update(self):
        ...
        if time.time() - self.pea_spawn >= 2 and zombie_on_line:  # если после последнего выпуска горошины прошло 2 секунды и зомби на горизонте
            print("Pea!")
            ...
            self.pea_spawn = time.time()  # обновляем время последнего выпуска пули
            #self.pea_spawn_sound.play()
...
class MyGame(arcade.Window):
    ...
    def update(self, delta_time):
        if self.game:
            ...
            if time.time() - self.zombie_spawn > 20:  # если со времени последней атаки прошло достаточно времени
